...
public class Tab implements Component {
    ...
    public void drawScreen(DrawContext context, double mouseX, double mouseY) {
        ...
        if(panel.isVisible()) {
            panel.drawScreen(context, mouseX, mouseY);
        }
        else {
            drawRoundedRect(context, x + 10, y, x + width - 10, y + height - 10, 15, 20, new Color(20, 20, 20, 190));
        }

        float textX = (x + (width / 2)) - (mc.textRenderer.getWidth(category.name)) - 15;
        drawString(context, category.name, textX, y + 10f, Color.white);
    }
