...
public class ModuleButton extends Component.PanelComponent {
    ...
    public void drawScreen(DrawContext context, double mouseX, double mouseY, double y) {
        ...
        drawRect(context, pX + 25, pY + pH + y, pX + pW - 25, pY + pH + y + height, new Color(40, 40, 40, 240).getRGB());
        drawString(context, module.getName() + (settings.isEmpty() ? "" : " ..."), pX + 30, pY + pH + y + 10, module.isEnabled() ? Color.GREEN : Color.white);

        if(isInside(mouseX, mouseY, pX + 25, pY + + pH + y, pX + pW - 25, pY + pH + y + oh)) {
            drawString(context, module.getDescription(),1, (mc.getWindow().getScaledHeight() * getGuiScale()) - mc.textRenderer.fontHeight * getGuiScale() - 10, Color.white);
        }

        if(expanded) {

            drawRect(context, pX + 25, pY + pH + y + oh - 5, pX + pW - 25, pY + pH + y + oh, Color.DARK_GRAY.getRGB());

            int offset = (int) oh;
            for (SettingComp setting : settings) {
                setting.drawScreen(context, mouseX, mouseY, y + offset);
                offset += 40;
            }
        }
    }
