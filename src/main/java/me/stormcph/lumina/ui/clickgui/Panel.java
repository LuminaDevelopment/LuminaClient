...
public class Panel implements Component {
    ...
    @Override
    public void drawScreen(DrawContext context, double mouseX, double mouseY) {
        ...
        drawRoundedRect(context, pX + 10, pY, pX + pW - 10, pY + pH + animation.getValue() - 10, 15, 20, new Color(20, 20, 20, 190));

        drawRect(context, pX + 30, pY + pH, pX + pW - 30, pY + pH + 3, new Color(100, 100, 100, (int) fadeAnim.getValue()).getRGB());
        double sf = 1f / getGuiScale();
        RenderUtils.enableScissor(pX * sf, (pY + pH) * sf, (pX + pW) * sf, (pY + pH + animation.getValue()) * sf);

        int offset = 25;
        for (ModuleButton button : buttons) {
            button.drawScreen(context, mouseX, mouseY, animation.getValue() - button.getHeight() - offset);
            offset += button.getHeight();
        }
        ...
        if(!ClickguiModule.performance.isEnabled()) {
            switch (category) {
                case COMBAT -> {
                    ...
                    drawImage(context, (pX + 5) * (1 / scale), (pY + 5) * (1 / scale), "textures/icons/combat.png");
                    matrices.pop();
                }
                case MOVEMENT -> {
                    matrices.push();
                    float scale = 0.027f;
                    matrices.scale(scale, scale, scale);
                    drawImage(context, (pX + 5) * (1 / scale), (pY + 1) * (1 / scale), "textures/icons/move.png");
                    matrices.pop();
                }
                case PLAYER -> {
                    matrices.push();
                    float scale = 0.025f;
                    matrices.scale(scale, scale, scale);
                    drawImage(context, (pX + 15) * (1 / scale), (pY + 5) * (1 / scale), "textures/icons/player.png");
                    matrices.pop();
                }
                case RENDER -> {
                    matrices.push();
                    float scale = 0.036f;
                    matrices.scale(scale, scale, scale);
                    drawImage(context, (pX + 15) * (1 / scale), (pY + 10) * (1 / scale), "textures/icons/render.png");
                    matrices.pop();
                }
                case MISC -> {
                    matrices.push();
                    float scale = 0.024f;
                    matrices.scale(scale, scale, scale);
                    drawImage(context, (pX + 15) * (1 / scale), (pY + 10) * (1 / scale), "textures/icons/misc.png");
                    matrices.pop();
                }
                case GHOST -> {
                    matrices.push();
                    float scale = 0.03f;
                    matrices.scale(scale, scale, scale);
                    drawImage(context, (pX + 15) * (1 / scale), (pY + 3) * (1 / scale), "textures/icons/ghost.png");
                    matrices.pop();
                }
            }
        }
